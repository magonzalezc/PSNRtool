...
def calculatePSNR_Y(original_y, encoded_y, npix):
...
def calculatePSNR(original_y, encoded_y, original_cb, encoded_cb, original_cr, encoded_cr, npix):
	...
	for i in range(0, len(original_cb)):
	...
	for i in range(0, len(original_cr)):
        ...
        meanSquaredErrorY = float(error_y) / float(npix) # And we get the mean squared error per pixel
        meanSquaredErrorCb = float(error_cb) / float(npix/4) # And we get the mean squared error per pixel
        meanSquaredErrorCr = float(error_cr) / float(npix/4) # And we get the mean squared error per pixel
	...
	print '', peakSignalToNoiseRatioY, ';', peakSignalToNoiseRatioCb, ';', peakSignalToNoiseRatioCr, ';'
